...
## 7. Verificación en PRODUCCIÓN (`intel.arroba.com`)

Smoke con `X-API-Key` tras el deploy — **todo 200 y datos vivos**:

| Endpoint | Estado |
|---|---|
| `POST /api/v2/company-intelligence/identity` | ✅ 200 |
| `POST /api/v1/financial-intelligence/analyze` | ✅ 200 |
| `GET /api/v1/company/{id}/ownership` | ✅ 200 (2 accionistas) |
| `GET /api/v1/company/{id}/governance` | ✅ 200 (55 cargos) |
| `GET /api/v1/company/{id}/events` | ✅ 200 |
| `GET /api/v1/company/{id}/ficha` | ✅ 200 |

Comprobado en la Ficha de Servier `B28184687`: `statements.cash_flow` presente, `ratios.roe.percentile = 75`,
`valuation.scenarios` presente, `ownership`/`governance` poblados. **Todo lo del plan I-1 + los endpoints
nuevos están LIVE en producción.**

---

*Documento generado en preview y verificado en producción (`intel.arroba.com`) con `X-API-Key` sobre
Servier `B28184687`, `B95222139` (PYME) y `A0051199H` (sin cuentas).*
