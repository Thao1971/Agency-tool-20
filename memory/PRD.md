...
- **Fases 0/1/2/4/5/6 (pack PENDING_FIXES de Daniel) aplicadas ✅ Preview (2026-09-01)** — 12 ficheros guardados en `memory/PENDING_FIXES/`, aplicados en orden (5 antes que 6). Solo preview, SIN desplegar.
  - **Fase 0** — 5 campos dormidos (domicilio, sit_mercantil, audited, balance_model, last_balance_year): reemplazados `services/data_layer/master/master_builder.py` (proyecta `registry` + `location.domicilio`) y `routes/company_intelligence.py` (contrato `/identity` + `_build`). Aditivo; los campos se pueblan tras `rebuild_master` (hoy null en e2e, esperado).
  - **Fase 1** — 2 endpoints legacy DocStudio (`/financial/company/{id}`, `/financial/compare`) migrados de `iberinform_financials` a `master_companies` vía `docstudio/data_access.py`. Sin callers conocidos.
  - **Fase 2** — bloque NUEVO `capital_markets` (BME/CNMV) en `routes/company_ficha.py` (+ helpers `_bme_listing_for`/`_cnmv_entity_for`), enganchado en `ficha()`; enciende chip "Cotizada" (identity.is_listed). E2E SERVIER: listing/buyers/is_listed=True ✓.
  - **Fase 4** — `_gather_iberinform()` en `sector_intelligence_v2.py` y `geo_intelligence.py` migrado a `master_companies`; retirada consulta muerta en `_gather_demography()`. Script `scripts/compare_sector_geo_master_vs_iberinform.py` ejecutado: **delta 0 en todo** (conteos idénticos 24.756/24.253; nota: corrido tras aplicar, refleja master-vs-master). Hallazgo: 2 provincias no resuelven a INE — `ARABA`(160), `CORUÑA (A)`(392) = 552 empresas fuera del conteo geo (candidatas a añadir al mapa BORME). JSON en `memory/PENDING_FIXES/fase4_comparison_result.json`.
  - **Fase 5** — `services/engines/financial/iberinform_ratios.py` (28 ratios) + `engine.py` expone `finances.iberinform_ratios`. Hoy null (dataset pre-24/07 sin `ratios_source:iberinform`; requiere re-ingesta `.tab`).
  - **Fase 6** — `services/engines/financial/pgc_account_labels.py` (198 etiquetados + 14 sin etiqueta) + `metrics.build_series_strict()` + `engine.py` expone `statements.detail` y `statements_consolidated`. E2E SERVIER: **detail=85 partidas**, consolidated **poblado** ✓.
  - **Verificación:** py_compile OK (11 ficheros), backend 200, imports OK, e2e `/ficha` con las 6 fases. Suite pytest completo se cuelga (~29%) y muchos tests sector/geo fallan — **CONFIRMADO PREEXISTENTE** (git stash de los 2 ficheros de Fase 4 → mismos fallos idénticos). Cero regresión introducida.
  - **PENDIENTE (usuario):** "adelante" explícito para desplegar. Para ver datos reales: Fase 0 → `rebuild_master`; Fase 5 → re-ingesta `.tab` del dataset 25k. Beta necesita dejar de descartar los campos nuevos + UI (fases 2/5/6).

- **HARDENING-038e: comité con prosa IA en BACKGROUND, cacheada por decision_id ✅ Preview (2026-08-29)** — sustituye el cap de 038d. Ahora `analyze()` NUNCA espera al LLM.
  - **`services/engines/investment_decision/engine.py`:** eliminado el cap `_NARRATIVE_TIMEOUT_S`; nuevos `_generate_narrative` (LLM en hilo, sin cap corto, red de seguridad `IDE_NARRATIVE_BG_TIMEOUT_S`=120s), `_apply_narrative`, `_bg_narrative` (tarea `asyncio.create_task` que genera y persiste). `analyze()`: (a) si `store.get(decision_id)` ya tiene `narrative_ai:true` → sirve la prosa florida AL INSTANTE (`narrative_status:cached`); (b) si no → devuelve prosa determinista YA (`generating`) y lanza la IA en segundo plano (guard `_narrative_inflight` anti-duplicado); (c) sin key → `deterministic`. La caché es el propio `store` (colección `investment_decisions`), sin colección nueva.
  - **Contrato nuevo para Beta:** la respuesta trae `meta.narrative_status` (`generating|ready|cached|deterministic|failed`), `meta.narrative_ai` y `meta.decision_id`. Beta muestra el veredicto YA; si `generating`, sondea `GET /api/v1/investment-decision/decision/{decision_id}` hasta `result.meta.narrative_status=="ready"` y cambia `executive_summary`/`investment_thesis` por la prosa florida.
  - **Verificado E2E (service-key, B28184687):** analyze frío **0,39s** (veredicto completo, prosa determinista, `generating`); sondeo → a ~20s `narrative_ai:true, ready` (prosa IA); analyze repetido **0,29s** con `narrative_ai:true, cached` (prosa florida instantánea, mismo `decision_id`). `decision_id` es determinista (cambia solo si cambia el veredicto) → la caché se invalida sola. Sin bloqueo del event loop (LLM en hilo). `py_compile` OK.
  - **PENDIENTE (usuario):** desplegar Intel + cablear en Beta el sondeo de `narrative_status`. `IDE_NARRATIVE_TIMEOUT_S` ya no se usa (queda `IDE_NARRATIVE_BG_TIMEOUT_S` opcional).
...
- **HARDENING-038d: comité rápido en el primer clic (<7,5s) ✅ Preview (2026-08-15)** — aplicado `INTEL_038d_comite_fast_v1.zip` (reemplazo de `investment_decision/engine.py`) PERO con corrección necesaria: **el parche del zip tal cual NO cumplía el objetivo** (medido 17-21s, no <7,5s).
  ...
  - **Verificado E2E (service-key, B28184687):** `analyze` 3× → **~4,2s** cada uno, HTTP 200, PROCEED/81, committee[10], reasoning, executive_summary presente, status ok, `narrative_ai:false` (prosa determinista). Ruta de éxito confirmada: con cap 30s → `narrative_ai:true`, audit ya NO crashea, prosa IA aplicada. `py_compile` OK.
  - **⚠️ SALVEDAD para el usuario:** con claude (~20s) y cap 4s, la prosa IA se salta SIEMPRE → se muestra la determinista (según spec del zip: "igual de correcta, solo menos florida"). Si quieren la prosa IA sin superar 7,5s: subir `IDE_NARRATIVE_TIMEOUT_S` (arriesga el cap), cambiar a proveedor más rápido, o mover a generación en background/caché (como `market.reading_ai`). Nota menor: el hilo LLM abandonado por timeout sigue ~20s en background y se autotermina (ok para QPS bajo del comité).
  - **PENDIENTE (acción del usuario):** desplegar Intel + Beta y confirmar que la pestaña Comité da veredicto al pulsar (no 502). No se tocó fork/Matching.

- **INTEL_deploy_prod_v1: sin código nuevo — smoke-test de prod-readiness en Preview ✅ (2026-08-15)** — el zip solo traía instrucciones de deploy; todo el código ya estaba aplicado (market.reading_ai, filtro ebitda_margin, valoración honesta, doc_type market_reading). Verificados en Preview con service-key (CIF B28184687) los 4 endpoints que Beta consume: (1) `POST investment-decision/analyze` → 200 PROCEED/81, comité de 10; (2) `GET signal-intelligence/succession-profile/{cif}` → 200, profile completo (administrator, family_business_probable, successor_candidate, succession_risk_score…); (3) `GET investment-intelligence/rollup-thesis` → 200, platform_candidate=true, payload completo; (4) `GET company/{cif}/ficha` → `market.reading_ai` no nulo (1.175 chars).
  - **⚠️ NOTA DE INTEGRACIÓN PARA BETA (rollup-thesis):** el endpoint espera el query param **`cnae_value`** (con `cnae_field` opcional, default `cnae_code`), NO `cnae_code`. `?cnae_code=2120` devuelve 400 "cnae_value is required"; la llamada correcta es `?cnae_value=2120`. Si la pestaña "Oportunidades · buy" sale vacía en prod, casi seguro es este nombre de parámetro en el proxy de Beta.
  - **PENDIENTE (acción del usuario):** desplegar Intel-140826 a prod + repetir este smoke-test contra `intel.arroba.com`. Recordatorio de seguridad del zip: rotar cuando toque el password de Atlas y `ARROBA_SERVICE_API_KEY` (prefijo `as_ace1afcc1`).
...
- **`market.reading_ai`: lectura de mercado IA (fact-lock, cacheada) cableada al bloque market ✅ Preview (2026-08-15)** — enganche pedido por el usuario tras aplicar la plantilla `market_reading`.
  ...
  - **Verificado E2E:** SERVIER 1ª llamada genera+cachea, 2ª 0,03s (cache hit); NESGAR cold 13,2s (Claude, <25s timeout) → 0,03s cacheada; lecturas distintas por empresa; `/ficha` incluye `market.reading_ai` (1.175 chars). Degradación: market no disponible / ctx vacío / fallo IA → `null` (el fallo NO se cachea). Fact-lock: todas las cifras (1/34 Madrid, 2/9 nacional, percentil 100, HHI 10.000, dinamismo 30/73) trazan al bloque market. **PENDIENTE: 1 redeploy** a prod (acción del usuario).

- **INTEL_para_emergent.zip: valoración honesta sin DFN + plantilla market_reading + script cobertura DN/EBITDA ✅ Preview (2026-08-15)** — aplicado por DIFF ADITIVO (el código en vivo ya iba por delante del zip: provenance/ranking/kpis_prior/narrativa/ID bridge REQ-004b intactos).
  - **Valoración honesta (`services/engines/financial/engine.py`, `valuation()` + `_valuation_full()`):** si el balance NO reporta `financial_debt` (típico de cuentas abreviadas/PYME), ya NO se asume deuda 0 restando la caja (inflaba el equity). Ahora `net_debt_known=false`, `net_debt=0` (Equity Value = EV, sin ajuste), hipótesis de aviso ("posible sobrevaloración si la empresa tiene deuda no desglosada") y −0,1 de confianza. Misma regla en la valoración simple y en los escenarios conservador/base/optimista (solo cambia el tratamiento de deuda; múltiplos/rangos/estructura intactos).
  - **`docstudio/model_provider.py`:** nueva plantilla `market_reading` en `generate_summary` (lectura de mercado 2-3 frases para comprador/analista M&A: posición sectorial + estado del sector + territorio + concentración). Aditivo; failover NVIDIA y `company_description` intactos.
  - **`scripts/coverage_dn_ebitda.py`** (nuevo, read-only): cuenta el techo de DN/EBITDA calculable. Ejecutado contra Preview: 13.493 empresas, deuda financiera 80,5%, caja 97,3%, EBITDA 72,4%, **DN/EBITDA calculable 61,6% (8.306)**. El 19,5% sin deuda desglosada es la cohorte que la valoración honesta ahora protege.
  - **Verificado E2E** (backend): `pytest test_skills_search_financial_filter.py` 9/9 verde; caso CON deuda → equity = EV − deuda neta, `net_debt_known:true`, conf 0,6; caso SIN deuda (MIGUEL ANGEL SOBRON EXPRESS) → equity == EV en valoración Y en los 3 escenarios, `net_debt_known:false`, aviso presente, conf 0,5. Solo código, sin paso de datos. **PENDIENTE: 1 redeploy** a prod (acción del usuario).
...
- **Bug fijado: "-15,4% interanual" repetido en la Tesis (contexto sectorial) ✅ Preview (2026-08-13)**
  ...
  - **Verificado E2E** (4 sectores): "-15,4%" desaparece de sector.narrative y thesis.narrative; el texto varía (dinamismo reducido/moderado/elevado por sector). El campo `national_yoy_pct` del card se deja (es dato nacional real; Beta decide ocultarlo/reetiquetarlo como "contexto nacional", según acordado). **PENDIENTE: 1 redeploy** (Daniel lo ve en prod).
- **Failover de narrativas NVIDIA (70B caído) ✅ Preview (2026-08-13)** — `docstudio/model_provider.py::_call_nvidia`.
  - Refactor a `_nvidia_once(prompt, model, timeout)` + `_call_nvidia` con **failover automático**: intenta el primario (`NVIDIA_MODEL`) con timeout corto `NVIDIA_PRIMARY_TIMEOUT` (12s por defecto) y, si falla/timeout, reintenta con `NVIDIA_MODEL_FALLBACK` (constante `NVIDIA_FALLBACK_MODEL="meta/llama-3.1-8b-instruct"`, tier GRATUITO). Cubre TODO lo que use provider=nvidia (Copilot, docstudio, descripción de ficha) sin tocar a cada llamador.
  - **Configurable / reversible:** para volver al 70B cuando NVIDIA lo restablezca → `NVIDIA_MODEL=meta/llama-3.3-70b-instruct` (el failover al 8b lo sigue protegiendo). Para latencia cero mientras el 70B esté caído → `NVIDIA_MODEL=meta/llama-3.1-8b-instruct`. **En código, así que funciona en prod SIN cambiar env** (el 70B configurado hace failover solo). Restricción cumplida: solo tier gratuito NVIDIA, sin modelos de pago (OpenAI/Anthropic).
  - **Verificado:** 70B primario (caído, ReadTimeout) → corta a ~12.7s → failover a 8b → `_call_nvidia` y `generate_copilot_message` devuelven contenido; descripción (8b directo) 0.5s sin penalización; `/ficha` intacto (description=ai). La ficha nunca dependió del 70B (ya usaba 8b vía `NVIDIA_DESC_MODEL`).
  - Nota: en Preview `IDE_NARRATIVE_PROVIDER=claude` y Copilot voice apagado → nada narrativo usaba nvidia aquí; el failover es el seguro para prod si apunta a nvidia.
- **Redistribución del Resumen — parte Intel/dato ✅ Preview (2026-08-13)** (spec `PARA_RESUMEN_REDISTRIBUCION.md`). Solo dato real; IA solo reformula.
  - **identity** (en `_build` de `company_intelligence.py` + enriquecido en el agregador `/ficha`): `verified` (bool = has_financials + origen registral), `auditor` (nombre desde rol Auditor del gobierno), `activity_es` (etiqueta CNAE ES vía `services/cnae_es.py` + `borme/cnae2025.json`; se mantiene `activity` EN como metadato), `linkedin_url` (null salvo enrichment; no se construye), `description` + `description_source ∈ {official, ai, web}`.
  - **Descripción (cascada + caché):** `services/company_summary.py::resolve_description` → (1) oficial registral en prosa `official`; (2) **IA Nvidia reformula el objeto social** `ai` (fact-lock: no añade personas/lugares/cifras/hechos; 2-3 frases CF); (3) `web_description` `web`; (4) null. **Caché en Mongo** `company_descriptions` por `master_id`+hash(objeto) → la IA no se llama en cada request (1ª ~3s, luego ~0.07s). Nueva fn `generate_company_description` en `docstudio/model_provider.py`. **El 70B (`NVIDIA_MODEL`) estaba caído/timeout en Nvidia → uso `meta/llama-3.1-8b-instruct` (override `NVIDIA_DESC_MODEL`) solo para esta reformulación; docstudio no se toca.**
  - **KPIs** (`compute_kpis` + `kpis_prior` en `engine.py`): añadidos `net_debt`, `net_debt_ebitda` (null si sin deuda/EBITDA = Empty honesto), `total_assets`, con `finances.kpis_prior` (año anterior, para ▲▼ YoY). Provenance: net_debt/net_debt_ebitda=calculated, total_assets=verified.
  - **Oportunidad** (`opportunity` en `/ficha`): `thesis` (prosa DETERMINISTA = sector.narrative + position.narrative + assessment.verdict, con `components` para explicabilidad) + `chips` [{enum,label_es}] derivadas del motor (buy_and_build/Buy & Build ← signal `ownership.consolidator`; capital_raise/Captación de capital ← signal growth.*; partner_entry/Entrada de socio ← accionista mayoritario + score≥50). Solo las que puntúan.
  - **Verificado E2E** (curl local + ingress) en SERVIER (source=ai, cache hit, 3 chips) + fallback (B83139154 con objeto→ai; sin chips) + endpoint standalone identity sin regresión. **DPD passthrough** (auditor/description reales; Beta anonimiza). **PENDIENTE: 1 redeploy** (código; la caché de descripción se llena sola en prod al primer request de cada empresa).
- **Procedencia por métrica (explicabilidad) ✅ Preview (2026-08-13, P2)** — flag `verified/calculated/inferred` por métrica.
  - **Shape:** `finances.provenance` = mapa por bloque `{kpis:{k:prov}, income_statement:{}, balance_sheet:{}, cashflow:{}, ratios:{}, ranking:{}}` + inline `finances.statements.cash_flow.rows[*].provenance` + `market.provenance` = `{sector:{}, geo:{}, concentration:{}, position:{}}`. Enums/valores crudos intactos (metadato, no PII → viaja en anónimo).
  - **Reglas:** verified = líneas reportadas de P&L/balance/EFE (revenue, ebit, net_income, todas las líneas de income_statement/balance_sheet y cf_* de cashflow); calculated = derivados (ebitda, márgenes, growth, CAGR, ratios[*], free_cash_flow, cash_conversion, percentiles/scores de ranking y market). Archivos: `services/engines/financial/engine.py` (`_build_provenance`/`_prov_statement`, aplicado en `analyze()`) y `routes/company_ficha.py` (`market()`).
  - **`inferred` NO aparece hoy** en estos bloques: ARROBA no estima estados financieros (política "solo años reales"); las empresas sin cuentas depositadas devuelven `has_financials:false` + `provenance:{}` + bloques vacíos (verificado: B83139154, A01001049). `inferred` queda reservado (solo se usa hoy en valoración: `multiple_basis:"inferred_reference"`, fuera de este alcance).
  - **Verificado E2E** (curl local + ingress) en SERVIER (con cuentas) y en 3 empresas sin cuentas. **PENDIENTE: 1 redeploy** (solo código, sin paso de datos).
- **Pasada de idioma CF a Señales + Oportunidades ✅ Preview (2026-08-13)** — cierra la semántica de toda la ficha (canon `CANON_NARRATIVA_CF_FICHA.md`, Anexo B).
  - **Señales (`GET /company/{id}/signals`):** por señal se emiten campos hermanos en prosa/ES conservando los enums crudos (type/category/polarity/severity) como metadato: `title` (titular en prosa desde la descripción, limpiando el umbral de máquina `(> 20%)` y localizando el decimal `24.4%→24,4%` + jerga `vs peers→frente a comparables`; ya NO se cae al ID de regla), `polarity_label` ES (positive→favorable, neutral→informativa, negative→desfavorable, negative+severidad crítica/warning/risk→de alerta), `category_label` ES (growth→Crecimiento, market→Mercado, operational→Operativo, ownership→Propiedad, financial→Finanzas, opportunity→Oportunidad, risk→Riesgo). Archivo: `routes/company_ficha.py` (`_SIGNAL_POLARITY_ES`/`_SIGNAL_CATEGORY_ES`/`_signal_polarity_label`/`_signal_title`). Verificado en SERVIER: los 6 signals con title+polarity_label+category_label poblados.
  - **Oportunidades (`POST /recommendation-intelligence/opportunities` y todo el motor de recomendación):** nombre de la tesis en prosa. A nivel set: `recommendation_type_label` ES. Por ítem: `recommendation_type_label`, `recommendation_role_label` (acquisition_target→Objetivo de adquisición, merger_candidate→Candidata a fusión, roll_up_candidate→Candidata a roll-up, etc.) y `title` prosa ("<nombre> — <tesis>"). Enums crudos conservados. Archivo: `services/engines/recommendation/engine.py` (`RECO_TYPE_ES`/`RECO_ROLE_ES`/`_reco_title` en `_build_rec`/`_wrap`/`matching`/`unavailable`). Aplica a comparables/buyers/sellers/opportunities/match. Verificado en SERVIER (set="Oportunidad de operación", ítems con title+role_label).
  - **Verificado E2E** (curl local + ingress). **PENDIENTE: 1 redeploy** para llevarlo a prod (solo código, sin paso de datos).
- **Materialización de aristas norm_ownership→master_relationships + labels 6/6 + role_label_es ✅ Preview (2026-08-12)**
  ...
  - **⚠️ CORRECCIÓN (2026-08-12):** verificado directamente contra prod (X-API-Key) → el DEPLOY DE CÓDIGO sí entró (nombres+labels OK en `/ficha`), pero las **aristas NO se materializaron en el Atlas de prod**: `MEDITERRANEAN /connections` = 1 (no 34). Causa: la instrucción previa `POST /data-layer/jobs` NO funciona porque la cola `data_layer_jobs` **no tiene worker** (`services.jobs.worker` no está en supervisor ni se arranca in-process, ni en Preview ni en prod) → el job quedó `queued` para siempre (error del agente al recomendar esa vía). **Fix:** nuevo endpoint SÍNCRONO `POST /api/v1/data-layer/rebuild-ownership-graph` (JWT admin) en `routes/data_layer.py`, patrón idéntico a `/rebuild-competitor-graph`, ejecuta `OG.rebuild_ownership_graph()` en proceso. Verificado en Preview (200, 1994 aristas, 235 ms). **PENDIENTE: 1 redeploy más** para activarlo en prod; luego el agente lo dispara y confirma MEDITERRANEAN=34.
  - **✅ RESUELTO EN PROD (2026-08-12):** tras el 2º redeploy, el agente ejecutó `POST /data-layer/rebuild-ownership-graph` contra prod (200, 4.7 s, 1994 aristas, 15 grupos) con la cuenta admin del usuario. Verificado en prod: **MEDITERRANEAN /connections = 34, NESGAR = 27, SERVIER = 2+2**; `/ficha` con `role_label_es`/`primary_driver_label`/`cash_flow_labels_es` OK. **Semántica + Propiedad + Gobernanza 100% live y verificadas en `intel.arroba.com`.**
