...
async def _process_batch(ncs: List[Dict], source: str, force: bool, stats: Dict) -> None:
    ...
    for nc in ncs:
        ...
        fields = {
            ...
            # Fase 0 (Daniel, 2026-09-01) · campos que ya llegan de Iberinform en norm_company
            # pero nunca se proyectaban a master_companies, así que la Ficha los mostraba vacíos
            # aunque el contrato de /identity ya los declaraba (address, mercantile_status) o
            # los añade ahora (audited, balance_model, last_balance_year). Puramente aditivo:
            # no cambia ningún campo/cálculo existente.
            "domicilio": (nc.get("address") or {}).get("domicilio"),
            "sit_mercantil": nc.get("sit_mercantil"),
            "audited": nc.get("audited"),
            "balance_model": nc.get("balance_model"),
            "last_balance_year": nc.get("last_balance_year"),
        }
        confidence = 0.9
        ...
        doc = {
            ...
            "location": {"provincia": canonical(prov["provincia"]), "municipio": canonical(prov["municipio"]),
                         ...
                         "domicilio": canonical(prov["domicilio"])},
            ...
            # Fase 0 (Daniel, 2026-09-01) · estado registral / cuentas depositadas, ya guardado
            # en norm_company desde la ingesta, nunca antes proyectado aquí.
            "registry": {
                "mercantile_status": canonical(prov["sit_mercantil"]),
                "audited": canonical(prov["audited"]),
                "balance_model": canonical(prov["balance_model"]),
                "last_balance_year": canonical(prov["last_balance_year"]),
            },
