...
- **v16 (2026-07-23) — DESPLEGADA Y VERIFICADA en Preview.** `arroba_agency_tool_v16.zip` (v13+v14+v15+documentación de esta capa) subida y desplegada por Neo. Testing agent: backend 19/20 (el único "fallo" es un bug de aserción del propio test, no del código — busca `real_companies` en la raíz cuando está anidado bajo `iberinform`; el dato es correcto), frontend 100%, 0 errores de consola en las 5 pantallas nuevas/tocadas. Post-deploy ejecutado en orden: `POST /migrate-dedupe` (9 grupos duplicados fusionados, 9 docs borrados — limpia el residuo del bug de v13), `GET /stats/view` confirmado (`total_signals=66050`, `total_opportunities=160`: 159 `ownership.consolidator` + 1 `opportunity.succession_signal`, coincide con lo que muestran Oportunidades/Señales), scheduler PLACSP confirmado arrancando en logs junto a BORME/DataComex/Watchlist/CNMV-BME. Regresión completa (fragmentation, ratios, rollup-thesis, watchlist, control-synergy, iberinform/stats, data-providers/health, auth negativa) → todo 200/401 correctos. `companies_master`/`master_companies`/`iberinform_companies` = 24.992 en los tres. Pantalla DIRCE renderiza (estado vacío hasta sincronizar INE/DIRCE, aún no ejecutado).
- **Nota operativa de Neo**: colocar los ficheros de test de integración fuera de `/app/backend/` (o `--reload-exclude tests/`) — uvicorn `--reload` vigila ese árbol y recarga al crearlos ahí. No afecta al runtime, solo higiene de despliegue.
- **Pendiente aún** (ninguno requiere código nuevo, son pasos operativos): sincronizar DIRCE/INE para poblar la pantalla de Demografía Empresarial; `POST /borme-link-backfill` (si se ingiere BORME real); `POST /rebuild-competitor-graph` (tras cada `/bootstrap`); recalibración periódica de `POST /baselines/compute`.
